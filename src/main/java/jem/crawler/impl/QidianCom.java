...
public class QidianCom extends AbstractCrawler implements CrawlerFactory {
    ...
    public Book getBook(String url, TypedConfig config) throws JemException, IOException {
        ...
        set(book, TITLE, queryText(doc, "div.book-information div.book-info h1 em"));
        val src = queryLink(doc, "a#bookImg img");
        if (src != null) {
            set(book, COVER, Flobs.forURL(new URL(src.replaceFirst("[\\d]+$", "600")), "image/jpg"));
        }
        set(book, AUTHOR, queryText(doc, "div.book-information div.book-info h1 a"));
        set(book, KEYWORDS, queryText(doc, "div.book-information div.book-info p.tag a", ";"));
        set(book, INTRO, Texts.forString(queryText(doc, "div.book-intro p", "\n")));
    ...
    private void getContents(Book book, Document doc, TypedConfig config) {
        for (val div : doc.select("div.volume-wrap div.volume")) {
            ...
            set(section, WORDS, queryText(div, "h3 cite"));
            book.append(section);
